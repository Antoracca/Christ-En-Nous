...
import { useFocusEffect } from 'expo-router'; // ✅ Import critique pour le timer strict
...
export default function BibleReader({
  ...
  // ✅ TIMER STRICT : Ne compte que si l'écran est affiché (Focus)
  useFocusEffect(
    useCallback(() => {
      // 1. QUAND ON ARRIVE (FOCUS)
      if (currentChapter && !loading) {
        // console.log('▶️ Reader Focus: Reprise du timer', currentChapter.book, currentChapter.chapter);
        try { 
            progress.switchTo(
                currentChapter.book, 
                currentChapter.chapter,
                currentChapter.verses.length // ✅ Total explicite
            ); 
        } catch {}
      }

      // 2. QUAND ON PART (BLUR) - Modal, Tab, etc.
      return () => {
        // console.log('⏸️ Reader Blur: Pause du timer');
