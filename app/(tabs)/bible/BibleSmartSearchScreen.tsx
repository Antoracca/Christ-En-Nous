...
type TokenKind = 'reference' | 'range' | 'phrase' | 'word' | 'approx';
...
function parseReferenceSmart(raw: string): SmartToken | null {
  ...
  try {
    ...
    if (parsed?.book && parsed?.chapter) {
      if (parsed?.end) {
        return { kind: 'range', raw, range: { start: parsed, end: parsed.end } };
      }
...
export default function BibleSmartSearchScreen() {
  ...
  const { searchVerses, isSearching, searchResults, clearSearch, navigateToChapter, bibleBooks, currentVersion } = useBible();
  ...
  const navigateFromRef = useCallback(async (ref: VerseRef) => {
    try {
      await navigateToChapter({ ...ref, end: undefined });
    } catch (e) {
