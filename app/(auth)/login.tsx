  ...
  React.useEffect(() => {
    const handler = setTimeout(async () => {
      ...
      if (routeParams?.email && value === routeParams.email.toLowerCase()) {
        console.log("💡 [LOGIN] Scénario post-changement d'email détecté via params. Validation spéciale activée.");
                  ...
                  {isLoading ? <ActivityIndicator size="large" color={theme.colors.primary} /> : (
                    <>
                      ...
                      <View style={styles.actionsContainer}>
                        <TouchableOpacity onPress={() => router.push('/(auth)/forgot-password')} disabled={isLockedOut}><Text style={styles.linkText}>Mot de passe oublié ?</Text></TouchableOpacity>
                      ...
                      <TouchableOpacity style={styles.registerContainer} onPress={() => router.push('/(auth)/register')} disabled={isLockedOut}><Text style={[styles.linkText, styles.registerLink]}>Première fois ? S&apos;inscrire</Text></TouchableOpacity>
                    </>
                  )}
                </Animated.View>
              </View>
